...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    username        = models.CharField(_('Username'), max_length=150, unique=True, validators=[username_validator], error_messages={'unique': _('The username is already exists')})
    email           = models.CharField(_('Email'), max_length=255, unique=True)
    first_name      = models.CharField(_('First Name'), max_length=150)
    last_name       = models.CharField(_('Last Name'), max_length=150)
    date_joined     = models.DateTimeField(_('Date Joined'), default=timezone.now)

    is_active       = models.BooleanField(_('Active Status'), default=True, help_text=_('Designates whether this user account should be considered active. Set this flag to False instead of deleting accounts.'))
    is_staff        = models.BooleanField(_('Staff Status'), default=False, help_text=_('Designates whether this user can access the admin site.'))
    is_superuser    = models.BooleanField(_('Superuser Status'), default=False, help_text=_('Designates that this user has all permissions without explicitly assigning them.'))

    type            = models.CharField(_('Type of User'), max_length=100, choices=USER_TYPE, default=CUSTOMER)
    image           = models.ImageField(upload_to=get_upload_path, width_field='width_field', height_field='height_field', null=True, blank=True)
...
class Distributor(models.Model):
    ...
    is_approved = models.BooleanField(default=False, verbose_name=_('Approve Status'), help_text=('Designates whether current status of this user approved by any admin'))
